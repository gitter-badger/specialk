        ...
        new PersistedMonadicKVDB[ReqBody, RspBody](MURI(here)) with Blobify with AMQPMonikerOps {
          ...
          class StringMongoDBManifest(override val storeUnitStr: String,
            ...
            override def asStoreValue(rsrc: mTT.Resource): CnxnCtxtLeaf[String, String, String] with Factual = {
              BasicLogService.tweet(s"In asStoreValue on $this for resource: $rsrc")
              val storageDispatch = rsrc match {
                case k: mTT.Continuation =>
                  BasicLogService.tweet(s"Resource $rsrc is a continuation")
                  continuationStorageType
                case _ =>
                  BasicLogService.tweet(s"Resource $rsrc is a value")
                  valueStorageType
              }
              BasicLogService.tweet(s"storageDispatch: $storageDispatch")
              val blob = storageDispatch match {
                ...
                case "CnxnCtxtLabel" =>
                  BasicLogService.tweet("warning: CnxnCtxtLabel method is using XStream")
                  toXQSafeJSONBlob(rsrc)
                case "XStream" =>
                  BasicLogService.tweet("using XStream method")
                  toXQSafeJSONBlob(rsrc)
                case _ =>
            ...
            def asCacheValue(ccl: CnxnCtxtLabel[String, String, String]): String = {
              BasicLogService.tweet("converting to cache value")
              ccl match {
                case CnxnCtxtBranch("string", CnxnCtxtLeaf(Left(rv)) :: Nil) =>
                  println(s"asCacheValue: $rv")
                  val unBlob = fromXQSafeJSONBlob(rv)
                  println(s"unBlob: $unBlob")
                  unBlob match {
                    case TheMTT.Ground(value) =>
                      BasicLogService.tweet(s"is this cheating? $value")
                      value
                    case rsrc: mTT.Resource =>
                      BasicLogService.tweet(s"did we actually get here?")
                      getGV(rsrc).getOrElse("WHOOPS!")
                  }
                case _ =>
            ...
            override def asResource(key: mTT.GetRequest, value: DBObject): emT.PlaceInstance = {
              BasicLogService.tweet(s"""PersistedMonadicKVDB :
                                        |method : asResource
                                        |this : $this
                                        |key : $key
                    ...
                    case Some(soln) =>
                      ...
                      if (compareNameSpace(ns, kvNameSpace)) {
                        ...
                        BasicLogService.tweet(s""" ******************************************
                                                  ...
                                                  | ****************************** """.stripMargin)
                        val finalRslt = emT.PlaceInstance(
                          k,
              ...
              BasicLogService.tweet(s"""PersistedMonadicKVDB :
                                        ...
                                        |value : $value
                                        |-----------------------------------------
                                        |returning : $computedRslt""".stripMargin)
              computedRslt
            }
          }

          override def asCacheK(ccl: CnxnCtxtLabel[String, String, String]): Option[mTT.Continuation] = {
            BasicLogService.tweet(s"converting to cache continuation stack$ccl")
            ccl match {
              case CnxnCtxtBranch("string", CnxnCtxtLeaf(Left(rv)) :: Nil) =>
