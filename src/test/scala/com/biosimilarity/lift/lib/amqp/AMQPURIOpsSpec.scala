...
class AMQPURIOpsSpec extends WordSpec with Matchers {
  ...
  "uriExchange" should  {
    ...
    "return the default exchange for an instance of URI containing no exchange" in {
      val testURINoExchange: URI =
        new URI("amqp", null, "localhost", 5672, null, null, null)
    ...
    "return the exchange of an instance of URI (2)" in {
      val testURIExchange02: URI =
        new URI("amqp", null, "localhost", 5672, "/mult/kult", "routingKey=routeroute", null)
      assert(uriExchange(testURIExchange02) == "mult")
    }

    "return the exchange of an instance of URI (3)" in {
      val testURIExchange03: URI =
        new URI("amqp", null, "localhost", 5672, "/mult/kult/jult", "routingKey=routeroute", null)
      assert(uriExchange(testURIExchange03) == "mult")
    }

    "return the exchange of an instance of URI (4)" in {
      val testURIExchange04: URI =
        new URI("amqp", null, "localhost", 5672, "/mult/kult/jult/iult", "routingKey=routeroute", null)
  ...
  "uriRoutingKey" should {
     ...
     *   val testURINoRoutingKey: URI =
     *     new URI("amqp", null, "localhost", 5672, "/mult", null, null)
     *   assert(uriRoutingKey(testURINoRoutingKey) == "routeroute")
     * }
     */

    "return the routing key of an instance of URI" in {
      val testURI02: URI =
        new URI("amqp", null, "localhost", 5672, "/mult", "routingKey=quux", null)
